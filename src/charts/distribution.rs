...
pub fn generate_distribution(date: &str) -> Result<String, Box<dyn std::error::Error>> {
    ...
    let mut svg = String::from("<svg width=\"400\" height=\"300\" xmlns=\"http://www.w3.org/2000/svg\">\n");
    svg.push_str("<style>\n");
    ...
    for (i, (actor, secs)) in stats.iter().enumerate() {
        let width = if total > 0 { (*secs as f64 / total as f64 * 300.0) as i32 } else { 0 };
