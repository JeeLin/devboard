    ...
    match args {
        DocArgs::Add {
            ...
            match document::create_document(&conn, task, &title, &doc_content) {
                Ok(doc) => println!(
                    "Created document '{}' (id: {}, task: {})",
                    doc.title, doc.id, doc.task_id
                ),
                Err(e) => eprintln!("Error: {}", e),
            }
        }
        DocArgs::List { task } => match document::list_documents_by_task(&conn, task) {
            Ok(docs) => {
                if docs.is_empty() {
        ...
        DocArgs::Get { id } => match document::get_document(&conn, id) {
            Ok(doc) => {
        ...
        DocArgs::Link { doc, task } => match document::link_document_to_task(&conn, doc, task) {
        ...
        DocArgs::ListTasks { id } => match document::list_tasks_by_document(&conn, id) {
            Ok(task_ids) => {
