...
impl ExternalSyncConfig {
    pub fn from_config() -> Option<Self> {
        let config_path = dirs::home_dir()?.join(".devboard").join("config.toml");
...
pub fn external_status() -> Result<String, String> {
    let config = ExternalSyncConfig::from_config().ok_or("External sync not configured.")?;
    ...
    match config.method.as_str() {
        ...
        "ssh" => Ok(format!("SSH sync: target: {}", config.target)),
